...
export default function BarxanPage() {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    try {
      ...
      const channel = supabase
        .channel('barxan-events')
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'events', filter: `type=eq.Barxan` },
          () => loadEvents()
        )
        .on(
          'postgres_changes',
  ...
  async function loadEvents() {
    try {
      setLoading(true);
      setError(null);
      const data = await getEventsByType('Barxan');
      setEvents(data);
    } catch (err) {
  ...
  async function handleCreateEvent(formData: {
    ...
    try {
      const result = await createEvent({ ...formData, type: 'Barxan' });
      if (result) {
        await loadEvents();
      } else {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        {/* Header Section */}
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary via-accent to-primary text-center">{t('barxan')}</h1>
